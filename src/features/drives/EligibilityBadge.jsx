...
export default function EligibilityBadge({ job, showDetails = false }) {
  ...
  if (result.status === "ELIGIBLE") {
    return (
      <div className="inline-flex flex-col items-start">
        <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700 shadow-xs">
  ...
  if (result.status === "PROFILE_INCOMPLETE" || result.status === "NO_PROFILE") {
    return (
      <div className="inline-flex flex-col items-start">
        <span
          className="inline-flex items-center gap-1.5 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700 shadow-xs"
  ...
  return (
    <div className="inline-flex flex-col items-start">
      <span
        className="inline-flex items-center gap-1.5 rounded-full border border-rose-200 bg-rose-50 px-2.5 py-1 text-xs font-semibold text-rose-700 shadow-xs"
      ...
      {showDetails && (
        <span className="mt-0.5 text-[11px] text-rose-600 font-medium">
          {result.message}
        </span>
      )}
    </div>
